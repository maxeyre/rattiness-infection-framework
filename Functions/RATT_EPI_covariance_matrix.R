# Function to extract the covariance matrix for MLE parameter estimates

get.covar.fn <- function(rat, human, control, par_hat){
  n.sim=11000; burnin=1000; thin=10
  ...
  hess_out <- numDeriv::hessian(MC.log.lik, par0)
  var.cov <- solve(-hess_out)
  out <- list(covariance_matrix = var.cov, hessian = hess_out)
  return(out)
}
