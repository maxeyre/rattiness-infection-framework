rattiness.epi.model.alt <- function(control, rat, human,
  ...
  omega2.0 <- control$omega2.0[1] # Additional spatial Gaussian process in human side - variance
  zeta0 <- control$zeta0[1] # Additional spatial Gaussian process in human side - scale of spatial correlation
  omega2_nugg0 <- control$omega2_nugg0[1] # Additional nugget effect in human side - variance
  ...
  # human
  par0.human <- control$par0.human[1] 
  # par0.human:
  # set = TRUE to start with regression coefficient estimates from glm and starting parameters set for xi0, omega2.0, zeta0 and omega2_nugg0
  # set = FALSE to start with all parameter values = 0
  # or input manually as c(regression coefficients) (and change xi0, omega2.0, zeta0 and omega2_nugg0 as required)
  
  if(is.logical(par0.human[1]) & par0.human[1] == TRUE){
    par0.human <- c(glm.par0.s)
  }
  if(is.logical(par0.human[1]) & par0.human[1] == FALSE){
    par0.human <- rep(0,p.human)
  }
  if(with_human_S==TRUE){
    par0.human <- c(par0.human,xi0,log(omega2.0),log(zeta0))
  } else{
    par0.human <- c(par0.human,xi0)
  }
  if(with_human_N==TRUE){
    par0.human <- c(par0.human,log(omega2_nugg0))
  }
  # full initial parameter set
  if(with_Ui == TRUE){
    par0 <- c(par0.rat,par0.human,log(psi0/(1-psi0)))
  }else {
    par0 <- c(par0.rat,par0.human)
  }
  ...
  while(iter.check > tol) {
    ...
    if(output.par==TRUE){
      x<-paste(model.name,"_task",task,"_iter",k-1,"_output.RDS",sep="")
      saveRDS(estim.par,file=outputfile(x))
    }
